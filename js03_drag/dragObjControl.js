...
function startDragControl(evt)
{

    if(ActiveElem&&!DraggingObj||ActiveControlCopy==true) //---prevents dragging conflicts on other draggable elements---
    {
        if(evt.target.getAttribute("id")=="activeElem" || evt.target.getAttribute("id")=="dragDot"||ActiveControlCopy==true)
        {
        ...
        if(objDragTarget)
        {
                ...
                if(objDragTarget.getAttribute("id")=="dragDot")
            {
                ...
                drawControlRxValue.value = rx
                drawControlRyValue.value = ry
...
function dragControl(evt)
{
    if(DraggingObj)
    {
        ...
        if(objDragTarget.getAttribute("id")=="dragDot" &&(DrawControl==true||EditControl==true))
        {
        ...
        else if((objDragTarget.getAttribute("id")=="activeElem")&& (DrawControl==true||EditControl==true))
        {
        ...
        else if(ActiveControlCopy==true)
        {
            var transformRequest = CopyControl.ownerSVGElement.createSVGTransform()
            //---attach new or existing transform to element, init its transform list---
            var myTransListAnim = CopyControl.transform
            var transList = myTransListAnim.baseVal
